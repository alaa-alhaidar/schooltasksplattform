          ...
          <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/55 p-4" onMouseDown={() => setSelectedAssignment(null)}>
            <section className="w-full max-w-lg rounded-3xl bg-white p-7 text-slate-950 shadow-2xl dark:bg-[#202833] dark:text-slate-100" onMouseDown={(event) => event.stopPropagation()}>
              <div className="flex items-start justify-between gap-4">
                <div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-100">{getSubjectLabel(selectedAssignment.subject, ku)}</span><h2 data-user-content className="mt-4 text-2xl font-black">{selectedAssignment.title}</h2></div>
              ...
              <div className="mt-6 space-y-5">
                ...
                {selectedAssignment.note && <div><p className="text-xs font-bold text-slate-400">{text.description}</p><p data-user-content className="mt-2 whitespace-pre-wrap leading-7">{selectedAssignment.note}</p></div>}
                {selectedAssignment.teacher_full_name && <div><p className="text-xs font-bold text-slate-400">{text.teacher}</p><p data-user-content className="mt-2 font-semibold">{selectedAssignment.teacher_full_name}</p></div>}
...
function AssignmentRow({ item, onClick, ku, text }: { item: Assignment; onClick: () => void; ku: boolean; text: Record<string, string> }) {
  return (
    <article role="button" tabIndex={0} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onClick(); }} className="today-task-card group w-full cursor-pointer rounded-2xl bg-emerald-100 p-5 text-start text-emerald-950 transition hover:-translate-y-1 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-emerald-500">
      <div className="flex items-start justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><h3 data-user-content className="text-lg font-black">{item.title}</h3><span className="rounded-full bg-white/70 px-2.5 py-1 text-[11px] font-bold">{getSubjectLabel(item.subject, ku)}</span></div>{(item.attachment_path || item.external_link) && <FileText className="today-task-file shrink-0 text-red-500" size={20} />}</div>
      {item.note && <p data-user-content className="today-task-note mt-3 text-sm font-medium leading-6 text-emerald-900/75">{item.note}</p>}
